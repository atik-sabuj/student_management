...
class RatingCommentWidget extends StatefulWidget {
  ...
  const RatingCommentWidget({Key? key, required this.featuresId, required this.onComplete})
      : super(key: key);
...
class _RatingCommentWidgetState extends State<RatingCommentWidget> {
  ...
  void _submit() async {
    ...
    final featuresRating = FeaturesRating(
      features_id: widget.featuresId,
    ...
    final didUserRating = await userProvider.didUserRate(widget.featuresId);
    if(didUserRating) {
      userProvider.updateRating(featuresRating)
          .then((value) {
        txtController.clear();
        focusNode.unfocus();
        widget.onComplete();
      ...
      userProvider.insertRating(featuresRating)
          .then((value) {
        txtController.clear();
        focusNode.unfocus();
        widget.onComplete();
